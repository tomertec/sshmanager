...
public interface IRemoteFileEditorService
{
    ...
    /// <param name="session">The editing session.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Result indicating success and whether changes were uploaded.</returns>
    Task<SaveResult> SaveToRemoteAsync(RemoteEditSession session, CancellationToken ct = default);
