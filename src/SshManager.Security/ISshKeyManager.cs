...
public interface ISshKeyManager
{
    ...
    /// <summary>
    /// Imports a PPK (PuTTY Private Key) file and converts it to OpenSSH format.
    /// </summary>
    /// <param name="ppkPath">Path to the PPK file.</param>
    /// <param name="passphrase">Optional passphrase if the key is encrypted.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>The converted key pair, or null if conversion failed.</returns>
    Task<SshKeyPair?> ImportPpkAsync(string ppkPath, string? passphrase = null, CancellationToken ct = default);
}
